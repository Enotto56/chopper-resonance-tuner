...
### 1. Устанавливаем скрипт калибровки на хост принтера. (клиппер перегрузится!)
```
   cd ~
   git clone https://github.com/MRX8024/chopper-resonance-tuner
   bash ~/chopper-resonance-tuner/install.sh
```
Если всё прошло хорошо, вы увидите папку - `adxl_results` в домашней директории вашего принтера (~/printer_data/config), в которую будут помещаться результаты калибровки, а также уже доступный макрос на панели макросов на главной странице Fluidd / Mainsail.
А если по какой-то причине нет, тогда установите [вручную](/wiki/manual_install_ru.md).

### Обработка собранных CSV-файлов на компьютере с Windows

1. Установите Python 3.10+ для Windows с [python.org](https://www.python.org/downloads/) и поставьте галочку «Add python.exe to PATH» в установщике. Git ставить не обязательно — репозиторий можно скачать ZIP-архивом.
2. Скопируйте папку репозитория (или хотя бы `chopper_plot.py` и `wiki/requirements.txt`) и все CSV с принтера на ПК. Сложите CSV в одну директорию (должны быть `stand_still.csv` и файлы, заканчивающиеся на `__.csv`).
3. Откройте **PowerShell** в папке репозитория и создайте/активируйте виртуальное окружение:
   ```powershell
   py -3 -m venv .venv
